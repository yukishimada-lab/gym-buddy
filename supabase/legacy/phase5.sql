-- ============================================================
-- gym-buddy: データベーススキーマ (Phase 5: 部位の整備 / 0kg 記録の調査と補正)
--
...
--
-- 実行順の前提: schema.sql → phase2.sql → phase3.sql → phase4.sql → phase5.sql
-- ============================================================
...
-- ============================================================
-- 3. 実行結果の確認
--    このスクリプトの最後に出る表で、直った件数と残っている件数が分かる。
-- ============================================================
select
  (select count(*) from public.exercises where muscle_group is null)
    as "部位が未設定の種目(0 が正常)",
  ...
  (select count(*) from public.workout_logs l
    ...
    as "重量を復旧できていない記録(0 が正常)",
  (select count(*) from public.workout_logs l
    where not exists (
      select 1 from public.workout_sets ws
      where ws.workout_log_id = l.id and ws.weight_kg > 0
    ))
    as "重量が入っていない記録(自重種目・ルーティン展開したままの記録)";
