-- ============================================================
-- gym-buddy / Phase 4: セット単位の記録 / 並べ替え / からだ記録の統合
--
-- 由来: もとの supabase/phase4.sql(現在は supabase/legacy/phase4.sql)
...
--
-- ★ マイグレーション化にあたっての変更点(2026-09):
--   一度きりのデータ移行(旧 workout_logs → workout_sets、
--   weight_logs / inbody_logs → body_logs)には
--   「移行先テーブルがまだ空のときだけ実行する」というガードを追加しました。
--   万一このファイルが再実行されても、ユーザーが後から削除した記録が
--   復活したり、編集した値が書き戻されたりしないようにするためです。
--   初回適用時の挙動は元の phase4.sql と同じです。
--
-- 実行順の前提: Phase 1 → Phase 2 → Phase 3 → Phase 4
-- ============================================================
...
--      workout_sets が空のとき(初回)だけ実行するので何度実行しても安全。
...
-- ガード: workout_sets がまだ 1 行も無い(= 初回移行)ときだけ展開する。
-- INSERT ... SELECT の副問い合わせは文の開始時点のスナップショットを見るため、
-- この文自身が挿入した行の影響を受けずに全記録が展開される。
where not exists (select 1 from public.workout_sets);
...
-- ------------------------------------------------------------
-- 3-1〜3-3. weight_logs / inbody_logs から body_logs への一度きりの移行
--
--   ガード: body_logs がまだ空のとき(= 初回移行)だけ実行する。
--   移行後にユーザーが削除・編集した内容が再実行で復活しないようにするため、
--   3 ステップ全体を 1 つの DO ブロックにまとめ、開始時点で 1 度だけ
--   「body_logs が空かどうか」を判定している。
--
--   3-1. weight_logs からの移行(同じ日付が既にあれば触らない)
--   3-2. inbody_logs からの移行(まだその日付の記録が無い場合)
--   3-3. 同じ日付に両方あった場合のマージ。
--        統合先で「まだ空(NULL)の項目」だけを InBody の値で埋める。
--        メモは重複しないときだけ「/」で連結する。
-- ------------------------------------------------------------
do $migrate_body_logs$
declare
  is_first_run boolean;
begin
  select not exists (select 1 from public.body_logs) into is_first_run;

  if not is_first_run then
    raise notice 'body_logs には既にデータがあるため、からだ記録の移行はスキップしました。';
    return;
  end if;

  -- 3-1. weight_logs からの移行
  ...
  -- 3-3. 同じ日付に両方あった場合のマージ(NULL の項目だけを埋める)
