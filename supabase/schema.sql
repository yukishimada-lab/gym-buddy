-- ============================================================
-- gym-buddy: データベーススキーマ (Phase 1)
--
-- Supabase ダッシュボードの「SQL Editor」にこのファイルの内容を
-- 貼り付けて「Run」を押すだけでセットアップできます。
-- 何度実行しても安全なように IF NOT EXISTS / OR REPLACE を使っています。
-- ============================================================
...
-- ============================================================
-- Phase 2 以降の拡張スキーマ(設計のみ・現時点では未使用)
-- 導入する際は下のコメントを外して実行してください。
-- ============================================================
...
--   name        text not null,           -- 食事内容(例: 鶏むね肉 200g)
--   protein_g   numeric(6, 1) not null default 0,  -- タンパク質 (g)
--   fat_g       numeric(6, 1) not null default 0,  -- 脂質 (g)
--   carbs_g     numeric(6, 1) not null default 0,  -- 炭水化物 (g)
--   calories    numeric(7, 1),           -- kcal(未入力なら PFC から計算)
--   created_at  timestamptz not null default now()
-- );
-- create index if not exists meals_user_date_idx on public.meals (user_id, meal_date desc);

-- -- 体重・InBody データ記録と目標サポート
...
--   muscle_mass_kg     numeric(5, 2),   -- 筋肉量 (InBody)
--   visceral_fat_level numeric(4, 1),   -- 内臓脂肪レベル (InBody)
--   memo               text,
--   created_at         timestamptz not null default now(),
...
-- ※ Phase 2 のテーブルを有効化する際は、上記と同様の RLS ポリシー
--    (auth.uid() = user_id) を必ず設定してください。
